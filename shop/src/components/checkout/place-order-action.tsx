...
export const PlaceOrderAction: React.FC<{
  ...
  // In-store pickup has no delivery fee.
  const effectiveShipping =
    isPickup || freeShippings ? 0 : verified_response?.shipping_charge!;
  const total = calculatePaidTotal(
    {
      totalAmount: subtotal,
      tax: verified_response?.total_tax!,
      shipping_charge: effectiveShipping,
    },
    Number(discount),
  );
  const handlePlaceOrder = () => {
    ...
    let input = {
      ...
      delivery_fee: effectiveShipping,
      total,
      delivery_time: isPickup ? undefined : delivery_time?.title,
      delivery_method,
      pickup_location: isPickup ? pickup_location : undefined,
      customer_contact,
      customer_name,
      note,
  ...
  let formatRequiredFields = isDigitalCheckout
    ...
    : isPickup
      ? [customer_contact, payment_gateway, billing_address, available_items]
      : [
          customer_contact,
          payment_gateway,
          billing_address,
