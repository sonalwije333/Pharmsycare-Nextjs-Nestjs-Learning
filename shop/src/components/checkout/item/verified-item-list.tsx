...
const VerifiedItemList: React.FC<Props> = ({ className }) => {
  ...
  const { price: shipping } = usePrice(
    verifiedResponse && {
      amount: isPickup ? 0 : verifiedResponse.shipping_charge ?? 0,
    }
  );
    ...
    ? calculatePaidTotal(
      {
        ...
        shipping_charge:
          isPickup || freeShippings ? 0 : verifiedResponse?.shipping_charge,
      },
    ...
    <div className={className}>
      ...
      <div className="mt-4 space-y-2">
        ...
        <div className="flex justify-between">
          <p className="text-sm text-body">{t('text-shipping')} <span className='text-xs font-semibold text-accent'>{isPickup ? '(In-store Pickup)' : freeShippings && `(${t('text-free-shipping')})`}</span></p>
