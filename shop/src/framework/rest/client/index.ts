...
class Client {
  ...
  types = {
    all: async (params?: Partial<TypeQueryOptions>) => {
      const response = await HttpClient.get<
        Type[] | { data?: Type[] }
      >(API_ENDPOINTS.TYPES, params);

      // Support both legacy array responses and paginator-style payloads.
  ...
  authors = {
    all: ({ name, ...params }: Partial<AuthorQueryOptions>) => {
      const search = HttpClient.formatSearchParams({
        name,
      });
  ...
  prescriptions = {
    ...
    getHistory: (id: number) =>
      HttpClient.get<PrescriptionHistoryEntry[]>(API_ENDPOINTS.PRESCRIPTIONS_HISTORY(id)),
  };
  supportTickets = {
    create: (input: CreateSupportTicketInput) =>
      HttpClient.post<SupportTicket>(API_ENDPOINTS.SUPPORT_TICKETS, input),
    getMy: (query: { page?: number; limit?: number; status?: string }) =>
      HttpClient.get<SupportTicketPaginator>(
        API_ENDPOINTS.SUPPORT_TICKETS_MY,
        query,
      ),
    reply: ({ id, ...input }: { id: number; message: string }) =>
      HttpClient.post<SupportTicket>(
        API_ENDPOINTS.SUPPORT_TICKET_REPLY(id),
        input,
      ),
  };
  refundReason = {
  ...
  faqs = {
    ...
    all: ({ faq_title, faq_type, issued_by, ...params }: Partial<FaqsQueryOptions>) =>
      HttpClient.get<FaqsPaginator>(API_ENDPOINTS.FAQS, {
        ...params,
        faq_type,
        issued_by,
        ...(faq_title ? { search: faq_title } : {}),
      }),
