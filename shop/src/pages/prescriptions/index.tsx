...
import PrescriptionHistoryTimeline from '@/components/prescriptions/prescription-history-timeline';
...
function PrescriptionHistorySection({ prescriptionId }: { prescriptionId: number }) {
  const [expanded, setExpanded] = useState(false);

  return (
    <div className="mt-4 border-t border-dashed border-border-200 pt-4">
      <button
        type="button"
      ...
      {expanded ? (
        <div className="mt-3">
          <PrescriptionHistoryTimeline prescriptionId={prescriptionId} />
        </div>
      ) : null}
    </div>
  );
              ...
              <div className="flex flex-col gap-4 sm:flex-row">
                ...
                <div className="flex-1">
                  ...
                  {prescription.medicines && prescription.medicines.length > 0 ? (
                    <div className="mt-3">
                      <p className="text-xs font-semibold text-heading">Medicines:</p>
                      <ul className="mt-1 flex flex-wrap gap-2">
                        {prescription.medicines.map((medicine) => (
                          ...
                          >
                            {medicine.name}
                            <span className="font-semibold">x{medicine.quantity}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
                  ...
                  <PrescriptionHistorySection prescriptionId={prescription.id} />
                </div>
              </div>
            </div>
          ))}
