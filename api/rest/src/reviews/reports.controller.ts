...
export class Report extends CoreEntity {
  ...
  @ApiProperty({ description: 'Model ID being reported', example: 1 })
  @Column()
  model_id: number;

  @ApiProperty({ description: 'Model type being reported', example: 'Marvel\\Database\\Models\\Review' })
  @Column()
  model_type: string;

  @ApiProperty({ description: 'Report message', example: 'This review contains inappropriate content' })
  @Column({ type: 'text' })
  message: string;
