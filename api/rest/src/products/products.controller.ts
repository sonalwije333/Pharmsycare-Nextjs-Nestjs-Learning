...
import { GetProductsExpiryDto } from './dto/get-products-expiry.dto';
...
@ApiTags('📦 Products')
@Controller('products-expiry')
export class ProductsExpiryController {
  constructor(private readonly productsService: ProductsService) {}

  @Get()
  @ApiOperation({ summary: 'Get products with expiry alerts' })
  @ApiResponse({
    status: 200,
    description: 'Products with expired or soon-to-expire drugs',
    type: ProductPaginator,
  })
  async getExpiryAlerts(@Query() query: GetProductsExpiryDto): Promise<ProductPaginator> {
    return this.productsService.getExpiryAlerts(query);
  }

  @Get('stats')
  @ApiOperation({ summary: 'Get expiry alert statistics' })
  @ApiResponse({
    status: 200,
    description: 'Counts of expired and expiring products',
  })
  async getExpiryStats(
    @Query('shop_id') shop_id?: number,
    @Query('days_before') days_before?: number,
  ) {
