...
export class OrdersService {
  ...
  async create(createOrderInput: CreateOrderDto): Promise<Order> {
    ...
    // Fulfillment: home delivery (default) or in-store pickup.
    const delivery_method =
      createOrderInput.delivery_method ?? FulfillmentType.DELIVERY;
    order.delivery_method = delivery_method;
    if (delivery_method === FulfillmentType.PICKUP) {
      order.pickup_location =
        createOrderInput.pickup_location ?? 'Sethma Pharmacy - Main Branch';
      order.delivery_fee = 0;
    } else {
      order.pickup_location = null;
      if (typeof createOrderInput.delivery_fee === 'number') {
