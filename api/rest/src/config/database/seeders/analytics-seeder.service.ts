...
// PharmSy-Care is a pharmacy: analytics should reference Medicine products only.
...
export class AnalyticsSeederService {
  ...
  private async seedTopRateProduct() {
    ...
    try {
      ...
      const normalizedTopRateProducts = topRateProducts
        .map((item: any) => {
        ...
        // Keep only Medicine products.
        .filter((item: any) => MEDICINE_PRODUCT_IDS.has(Number(item.product_id)));
