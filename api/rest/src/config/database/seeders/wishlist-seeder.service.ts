...
export class WishlistSeederService {
  ...
  async seed() {
    try {
      ...
      // PharmSy-Care is a pharmacy: build wishlists from the seeded Medicine
      // products in the database instead of the legacy hardcoded catalog.
      const products = await this.productRepository.find({ take: 40 });
