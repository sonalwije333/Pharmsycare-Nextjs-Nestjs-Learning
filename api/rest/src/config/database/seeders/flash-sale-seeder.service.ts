...
// PharmSy-Care is a pharmacy: only flash-sale Medicine products.
  ...
  private async seedFlashSales() {
    ...
    try {
      ...
      for (const flashSaleData of data) {
        // Extract product IDs from products array if available, keeping only
        // Medicine products.
        let productIds: number[] = [];
        if (flashSaleData.products && Array.isArray(flashSaleData.products)) {
          productIds = flashSaleData.products
            .map((p) => Number(p.id))
            .filter((id) => MEDICINE_PRODUCT_IDS.has(id));
        }

        // Skip flash sales that have no Medicine products.
