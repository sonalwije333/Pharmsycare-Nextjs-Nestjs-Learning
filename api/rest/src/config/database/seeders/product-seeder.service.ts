...
export class ProductSeederService {
  ...
  async seed(): Promise<void> {
    ...
    try {
      ...
      // PharmSy-Care is a pharmacy: only seed Medicine products.
      const medicineProductsJson = (productsJson as any[]).filter(
        (product: any) => product.type?.slug === 'medicine',
      );

      // Seed main products
      const products = plainToClass(Product, medicineProductsJson as object[]);
