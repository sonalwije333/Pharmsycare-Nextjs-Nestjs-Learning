...
export class ConversationsSeederService {
  ...
  private async seedConversations() {
    ...
    try {
      ...
      // PharmSy-Care is a pharmacy with a single Medicine shop. Point every
      // seeded conversation at that shop instead of the legacy mixed shops.
      const medicineShopRows = await this.conversationRepository.manager.query(
        `SELECT id FROM shops WHERE slug = 'medicine' OR LOWER(name) = 'medicine' LIMIT 1`,
      );
      const medicineShopId = medicineShopRows?.[0]?.id ?? null;
      if (!medicineShopId) {
        this.logger.warn(
          '⚠️ Medicine shop not found, skipping conversations seeding...',
        );
        return;
      }
