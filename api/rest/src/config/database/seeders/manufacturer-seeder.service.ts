...
export class ManufacturerSeederService {
  ...
  async seed() {
    try {
      ...
      // PharmSy-Care is a pharmacy: only seed Medicine manufacturers.
      const medicineManufacturers = (manufacturersJson as any[]).filter(
        (item: any) => item.type?.slug === 'medicine',
      );

      // Map JSON data to Manufacturer entities with guaranteed unique name/slug
      const manufacturers = this.mapManufacturersWithUniqueFields(
