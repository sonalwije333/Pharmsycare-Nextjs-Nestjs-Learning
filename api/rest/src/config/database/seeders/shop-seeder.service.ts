...
export class ShopSeederService {
  ...
  private getShopsData(): Partial<Shop>[] {
    ...
    // PharmSy-Care is a pharmacy: only seed the Medicine shop.
    const medicineShops = (shops as any[]).filter(
      (shop: any) =>
        shop.slug === 'medicine' || shop.name?.toLowerCase() === 'medicine',
    );
    return medicineShops.map((shop: any) => ({
      id: shop.id,
  ...
  async seed(): Promise<void> {
    try {
      ...
      // Additional (non-medicine) demo shops are intentionally not seeded
      // because PharmSy-Care operates as a single pharmacy.
