...
export class AuthorsSeederService {
  ...
  async seed() {
    // PharmSy-Care is a pharmacy. "Authors" is a books-vertical concept and is
    // not applicable here, so the table is only cleared and nothing is seeded.
    this.logger.log('🌱 Starting authors seeder...');

    try {
      await this.clear();
      this.logger.log(
        'ℹ️ Skipping author seeding (not applicable to a pharmacy scenario)',
      );
    } catch (error) {
      this.logger.error('❌ Failed to clear authors:', error.message);
      throw error;
    }
  }
