...
export class TypeSeederService {
  ...
  private getTypesData(): Partial<Type>[] {
    ...
    // PharmSy-Care is a pharmacy: only seed the Medicine type.
    const medicineTypes = (types as any[]).filter(
      (type: any) => type.slug === 'medicine',
    );
    return medicineTypes.map((type: any) => ({
      id: type.id,
      name: type.name,
      slug: type.slug,
      icon: type.icon,
