...
const ProductInventoryList = ({
  ...
  let columns = [
    ...
    },
    {
      ...
      align: 'center',
      width: 180,
      render: (expiryDate: string | null | undefined, record: Product) => {
        if (!expiryDate) {
          return <span className="text-body">-</span>;
        }

        const daysUntilExpiry = record.days_until_expiry;
        const isExpired = record.expiry_status === ProductExpiryStatus.EXPIRED;
        const isExpiringSoon =
          record.expiry_status === ProductExpiryStatus.EXPIRING_SOON;
        ...
        return (
          <div className="flex flex-col items-center gap-1">
            ...
            {isExpiringSoon && typeof daysUntilExpiry === 'number' ? (
              <Badge
                text={`${daysUntilExpiry} day(s) left`}
                color="bg-status-processing/10 text-status-processing"
                animate
                className="capitalize"
              />
            ) : null}
