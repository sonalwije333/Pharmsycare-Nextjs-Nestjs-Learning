...
import { TaxPaginator, TaxQueryOptions } from '@/types';
...
export const useTaxesQuery = (options: Partial<TaxQueryOptions> = {}) => {
  const { data, error, isLoading } = useQuery<TaxPaginator, Error>(
    [API_ENDPOINTS.TAXES, options],
    ({ queryKey, pageParam }) =>
      taxClient.paginated(Object.assign({}, queryKey[1], pageParam)),
    {
