...
export const useProductExpiryQuery = (options: Partial<ProductQueryOptions>) => {
  const { data, error, isLoading } = useQuery<ProductPaginator, Error>(
    [API_ENDPOINTS.PRODUCTS_EXPIRY, options],
    ({ queryKey, pageParam }) =>
      productClient.expiryAlerts(Object.assign({}, queryKey[1], pageParam)),
    {
      keepPreviousData: true,
    },
  );
...
export const useProductExpiryStatsQuery = (options: {
  shop_id?: string | number;
  days_before?: number;
}) => {
  const { data, error, isLoading } = useQuery(
    [API_ENDPOINTS.PRODUCTS_EXPIRY_STATS, options],
    () => productClient.expiryStats(options),
    {
      keepPreviousData: true,
    },
  );
